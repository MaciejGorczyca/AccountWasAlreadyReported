...
namespace ChallengesAreEvil
{
    ...
    class LeagueConnection
    {
        ...
        public async Task<dynamic> Post(string url, string body)
        {
            if (!connected) throw new InvalidOperationException("Not connected to LCU");

            var res = await HTTP_CLIENT.PostAsync("https://127.0.0.1:" + processInfo.Item3 + url, new StringContent(body, Encoding.UTF8, "application/json"));
            var stringContent = await res.Content.ReadAsStringAsync();
